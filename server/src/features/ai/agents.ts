...
export interface CompilerResult {
  resumeText: string;
  atsAnalysis: AgentResponse;
  skillAnalysis: AgentResponse;
  experienceAnalysis: AgentResponse;
  compiledInsights: {
    overallScore: number;
    strengths: string[];
    weaknesses: string[];
...
// ============================================
export const atsAgent = async (resumeText: string): Promise<AgentResponse> => {
...
// ============================================
export const skillAnalysisAgent = async (resumeText: string): Promise<AgentResponse> => {
  const startTime = Date.now();
...
// ============================================
export const experienceAgent = async (resumeText: string): Promise<AgentResponse> => {
  const startTime = Date.now();
...
export const runMultiAgentAnalysis = async (
  ...
  try {
    ...
    const [atsResult, skillResult, expResult] = await Promise.all([
      atsAgent(resumeText),
      skillAnalysisAgent(resumeText),
      experienceAgent(resumeText),
    ]);

    // Run compiler with results from other agents
    const compilerResult = await compilerAgent(
      resumeText,
      atsResult,
      skillResult,
      expResult
...
const mockCompilerResponse = () => ({
  overallAssessment:
    'Strong mid-level engineer with excellent technical foundation and emerging leadership qualities. Ready for senior role with focus on architecture and system design.',
  overallScore: 82,
  strengths: [
  ...
  sixMonthPlan:
    'Focus on senior-level technical skills: complete system design course, build 2 architectural projects, improve resume impact metrics, and network with senior engineers.',
  oneYearPlan:
    'Transition to Senior Engineer or Tech Lead role, develop architectural expertise, establish thought leadership through articles/speaking, and mentor junior developers.',
  resources: [
