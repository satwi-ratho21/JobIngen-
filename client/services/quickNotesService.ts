...
${input.content.substring(0, 4000)}`;
        ...
        // prefer calling backend so API key stays server‑side
        if (window.location.origin) {
            try {
                ...
                console.warn('Backend call failed, falling back to direct client API or mock', err);
            }
        }

        // continue with direct client request or mock below
