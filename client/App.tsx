...
import ATSCandidateRanking from './component/ATSCandidateRanking';
...
const App: React.FC = () => {
  ...
  // Study Plan data
  const [studyPlanData, setStudyPlanData] = useState<{ company: string; customCompanyName?: string; role: string } | null>(null);
  ...
  const navigateToStudyPlan = (company: string, customCompanyName: string | undefined, role: string) => {
    setStudyPlanData({ company, customCompanyName, role });
    ...
    <div className={`flex h-screen font-sans overflow-hidden ${darkMode ? 'bg-slate-950 text-slate-100' : 'bg-[#f8fafc] text-slate-900'}`}>
      ...
      `}>
        ...
        <nav className="flex-1 p-4 space-y-1 overflow-y-auto custom-scrollbar">
            ...
            <NavItem view="lab" icon={Beaker} label="ATS Candidate Ranking" />
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden relative">
        ...
        <div className={`flex-1 overflow-auto p-4 lg:p-6 ${darkMode ? 'bg-slate-950' : 'bg-[#f8fafc]'}`}>
          ...
          {currentView === 'tech-accelerator' && <TechAccelerator onNavigate={setCurrentView} onNavigateWithProfile={navigateToEarlyWarningWithProfile} navigateToStudyPlan={navigateToStudyPlan} />}
          {currentView === 'study-plan' && studyPlanData && <StudyPlan onNavigate={setCurrentView} company={studyPlanData.company} customCompanyName={studyPlanData.customCompanyName} role={studyPlanData.role} />}
          ...
          {currentView === 'lab' && <ATSCandidateRanking />}
